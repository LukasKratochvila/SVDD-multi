...
class MYDATA_Dataset(TorchvisionDataset):
    ...
    def __init__(self, root: str, version: int, Res: int = 32,
                 validation: bool = False, autoaugment: bool = False):
        super().__init__(root)

        self.n_classes = 4 # 0: normal, 1: outlier
        self.name = "MYDATA_Dataset"
        ...
        avg_min_max=np.average(min_max,0)
        ...
        trans=[transforms.Resize((Res,Res)),#160,120
               ...
               transforms.Normalize([avg_min_max[0]] * 3,
                                    [avg_min_max[1] - avg_min_max[0]] * 3)]
        trans=trans if autoaugment else [trans[0]]+trans[2:]
        trans=trans[1:] if Res == 0 else trans
        transform = transforms.Compose(trans)

        target_transform = transforms.Lambda(lambda x: x-1)#transforms.Lambda(lambda x: one_hot_embedding(x, self.n_classes))

        train_set = MyData(root=self.root, train=True, version=version,
                              transform=transform, target_transform=target_transform)
        # Subset train set to normal class
        #train_idx_normal = get_target_label_idx(train_set.train_labels, self.target_class)
        #self.train_set = Subset(train_set, train_idx_normal)elf.train_set
...
class MyData(data.Dataset):
    ...
    """
    base_folder_list = ['100-eeict-batches-py','200-eeict-batches-py','300-eeict-batches-py','400-eeict-batches-py']
    version_list = [100, 200, 300, 400]
    base_folder = None
    train_list = None
    test_list = None
    ...
    def _load_hashes(self, version):
        assert version in self.version_list, "Wrong version of dataset"
