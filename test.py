...
model.set_network(cfg.settings['net_name'])
# If specified, load Deep SVDD model (radius R, center c, network weights, and possibly autoencoder weights)
model.load_model(model_path=xp_path+load_model, load_ae=False)

dataset = load_dataset(dataset_name, cfg.settings['data_path'])
model.test(dataset,cfg.settings['device'],cfg.settings['n_jobs_dataloader'], cfg.settings['batch_size'])
...
# # Plot ROC curves
# fpr_1,tpr_1, thresholds_1 = roc_curve([x[1] for x in model.results['test_scores']],
#                                 [x[2] for x in model.results['test_scores']])
...
# print('Dataset: %s' % dataset_name)
# print("Test AUC: {0:.3f}%".format(100. * model.results['test_auc']))
# print("The first threshold after tpr == 0.9 is: {} ".format(thresh_1))
# [[TP,FN],[FP,TN]] = confusion_matrix([x[1] for x in model.results['test_scores']],
#                                      [x[2] >= thresh_1 for x in model.results['test_scores']])
...
#     print("Test AUC: {0:.3f}%".format(100. * model.results['test_auc']), file=file)
